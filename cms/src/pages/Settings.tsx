...
import { User, Lock, Save, CheckCircle, Loader2, Layout } from 'lucide-react';
...
export const Settings = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Fallback for profile if API fails but auth store has name
        if (authName) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
                ...
                <div className="pt-6">
                    ...
                    <form onSubmit={handleChangePassword} className="space-y-3">
                        <input
                            ...
                            className="w-full border-2 border-gray-50 rounded-xl p-3 outline-none focus:border-saffron font-bold text-navy transition-all"
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 hover:shadow-xl transition-all">
            ...
            <div className="space-y-6">
                ...
                <button
                    ...
                    className="w-full bg-ashoka text-white py-4 rounded-xl font-black uppercase tracking-[0.2em] text-xs shadow-lg hover:brightness-110 active:scale-95 transition-all"
                >
                   Broadcast New Branding
                </button>
            </div>
          </div>
      </div>
      ...
      {message && (
          <div className="fixed bottom-10 left-1/2 -translate-x-1/2 bg-green-600 text-white px-8 py-3 rounded-full font-black uppercase tracking-widest text-xs shadow-2xl animate-bounce z-50 flex items-center gap-2">
